...
// Compiler-agnostic deprecated macro
    ...
    #define DEPRECATED_BWD __attribute__((deprecated))
#elif defined(_MSC_VER)
    #define DEPRECATED_BWD __declspec(deprecated)
#else
    #define DEPRECATED_BWD
#endif
...
DEPRECATED_BWD float* backward_output_layer(const OutputLayer* layer, const float* d_loss_d_output_logits, const float* hidden_state_input, int model_dim, int vocab_size, LegacyLLM_Gradients* grads);
...
DEPRECATED_BWD float* backward_feed_forward(const FeedForwardLayer* layer, const float* d_loss_d_ffn_output, const float* ffn_input, const float* hidden_pre_relu_output, int model_dim, int block_idx, LegacyLLM_Gradients* grads);
...
DEPRECATED_BWD float* backward_layer_norm(const float* d_loss_d_norm_output, const float* input_vec, const float* norm_gamma, const float* norm_beta, float mean, float inv_std_dev, int size, int block_idx, int norm_idx, LegacyLLM_Gradients* grads);
...
DEPRECATED_BWD float* backward_multi_head_attention(const MultiHeadAttentionLayer* layer, const float* d_loss_d_mha_output, const float* mha_input, const float* query_vec_pre_bias, const float* key_vec_pre_bias, const float* value_vec_pre_bias, const float* query_vec, const float* key_vec, const float* value_vec, const float* attention_output_vec, int model_dim, int block_idx, LegacyLLM_Gradients* grads);
...
DEPRECATED_BWD float* backward_transformer_block(const TransformerBlock* block, const float* d_loss_d_block_output, int model_dim, int block_idx, LegacyLLM_Gradients* grads, const TransformerBlockContext* context);
...
DEPRECATED_BWD void backward_embedding(const EmbeddingLayer* layer, int token_id, const float* d_loss_d_embedding_output, int model_dim, LegacyLLM_Gradients* grads);
...
DEPRECATED_BWD void backward_llm(LegacyLLM* model, int token_id, int true_token_id, LegacyLLM_Gradients* grads);
